...
type TemplateFormat = 'json' | 'csv' | 'xlsx';
...
export class BankDetailComponent {
  ...
  // ── Template download ─────────────────────────────
  readonly templateFormat = signal<TemplateFormat>('json');
  ...
  /** Genera y descarga una plantilla de ejemplo en el formato seleccionado. */
  downloadTemplate() {
    ...
    return [
      {
        ...
        option1: 'Opción A (Respuesta correcta)',
        option2: 'Opción B',
        option3: 'Opción C',
        option4: 'Opción D',
  ...
  private downloadJsonTemplate() {
    const data = this.getTemplateData().map((row) => ({
