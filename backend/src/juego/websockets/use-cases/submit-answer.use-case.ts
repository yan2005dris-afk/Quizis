...
export class SubmitAnswerUseCase {
  ...
  async execute(payload: AnswerPayload) {
    this.logger.log(`Procesando respuesta para sala ${payload.tokenCompartido}, pregunta ${payload.preguntaId}`);

    // 1. Obtener la pregunta activa de Redis para validación ultra-rápida
    const activeQuestion = await this.cacheService.getActiveQuestion(payload.tokenCompartido);

    if (!activeQuestion || activeQuestion.preguntaId !== payload.preguntaId) {
      throw new BadRequestException('La pregunta no está activa o el ID no coincide.');
    }

    // 2. Verificar estado en Redis
    const status = await this.cacheService.getQuestionStatus(payload.tokenCompartido);
    if (status === 'answered') {
      throw new BadRequestException('Esta pregunta ya fue respondida.');
    }

    // 3. Validar si la opción es correcta
    const opcion = activeQuestion.opciones.find((o: any) => o.opcionId === payload.opcionId);
    if (!opcion) {
      throw new NotFoundException('La opción seleccionada no pertenece a esta pregunta.');
    }
    ...
    await this.cacheService.setQuestionStatus(payload.tokenCompartido, 'answered');
    ...
    return {
      ...
      feedback: esCorrecta ? activeQuestion.feedbackCorrecto : activeQuestion.feedbackIncorrecto,
    };
  }
}
