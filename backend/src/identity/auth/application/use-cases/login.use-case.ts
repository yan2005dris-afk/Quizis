...
import type { UserRecord } from '../../../users/domain/repositories/user.repository';
...
export class LoginUseCase {
  ...
  private async validateUser(loginUserDto: LoginUserDto): Promise<UserRecord> {
    const { email, password } = loginUserDto;
    const user = await this.userRepo.findByEmailIncludingDeleted(email);
